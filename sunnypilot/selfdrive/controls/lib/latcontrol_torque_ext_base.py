...
class LatControlTorqueExtBase:
  def __init__(self, lac_torque, CP, CP_SP, CI):
    ...
    self.friction_look_ahead_v = [1.2, 2.5]  # how many seconds in the future to look ahead in [0, ~2.1] in 0.1 increments
    self.friction_look_ahead_bp = [9.0, 36.0]  # corresponding speeds in m/s in [0, ~40] in 1.0 increments

    # Scaling the lateral acceleration "friction response" could be helpful for some.
    # Increase for a stronger response, decrease for a weaker response.
    self.lat_jerk_friction_factor = 0.2
    self.lat_accel_friction_factor = 0.85  # in [0, 3], in 0.05 increments. 3 is arbitrary safety limit
